...
class ControllerExtensionPaymentHipsCheckout extends Controller
{
    ...
    public function index()
    {
        ...
        if ($this->request->get['route'] == "extension/payment/hips_checkout") {
            
            $this->document->setTitle($this->language->get('heading_title'));
            
            $data['column_left'] = $this->load->controller('extension/payment/hips_checkout/left');
            $data['column_main'] = $this->load->controller('extension/payment/hips_checkout/main');
            ...
            return $this->load->view('extension/payment/hipsform', $data);
    ...
    public function main()
    {
        ...
        list($totals, $taxes, $total) = $this->model_extension_payment_hips_checkout->getTotals();
        ...
        if ($this->model_extension_payment_hips_checkout->checkForPaymentTaxes($products)) {
            $redirect = $this->url->link('checkout/cart');
        }
        ...
        if ((!isset($this->session->data['shipping_method']) || empty($this->session->data['shipping_method'])) && (isset($this->session->data['shipping_methods']) && !empty($this->session->data['shipping_methods']))) {
            $this->session->data['shipping_method'] = $this->model_extension_payment_hips_checkout->getDefaultShippingMethod($this->session->data['shipping_methods']);
        }
        ...
        if (!$redirect) {
            ...
            if (isset($this->session->data['hips_checkout_currency']) && $this->session->data['hips_checkout_currency'] != $this->session->data['currency']) {
                $this->model_extension_payment_hips_checkout->log('Currency changed, unsetting HC order id');
                unset($this->session->data['hips_checkout_order_id']);
            ...
            if (isset($this->session->data['order_token']) && isset($this->request->post['type']) && $this->request->post['type'] == 'partsection') {
                ...
                $respnse = $this->model_extension_payment_hips_checkout->orderUpdate($this->config->get('hips_key'), $hips_order_data, $this->session->data['order_token']);
                ...
                if ($respnse) {
                    ...
                    $this->model_extension_payment_hips_checkout->updateOrder($this->session->data['order_id'], $hips_checkout['order_id'], $encrypted_order_data, json_encode($respnse));
                }
                ...
                $this->createOrder();
                list($hips_order_data, $encrypted_order_data) = $this->hipsOrderData();
                $this->model_extension_payment_hips_checkout->log('Order Created');
                $this->model_extension_payment_hips_checkout->log($hips_order_data);
                
                unset($this->session->data['hips_checkout_data']);
                
                $respnse = $this->model_extension_payment_hips_checkout->orderCreate($this->config->get('hips_key'), $hips_order_data);
            ...
            ?>
              <?php
                ...
                if (isset($respnse->id) && $respnse->id != '') {
                    ...
                    $this->model_extension_payment_hips_checkout->addOrder($this->session->data['order_id'], $hips_checkout_order_id, $encrypted_order_data, json_encode($respnse));
                } else {
        ...
        return $this->load->view('extension/payment/hips_checkout_main', $data);
    ...
    public function callback()
    {
        ...
        if (trim($data->event) == 'payment.successful')
        {
            ...
            if (isset($res)) {
                ...
                $hips_checkout_order = $this->model_extension_payment_hips_checkout->getOrder($res);
                ...
                if ($hips_checkout_order) {
                    ...
                    if ($order_info) {
                        ...
                        $order = $this->model_extension_payment_hips_checkout->orderRetrieve($this->config->get('hips_key'), $res);
                        
                        $this->model_extension_payment_hips_checkout->log('Order details from push:');
                        $this->model_extension_payment_hips_checkout->log($order);
                        
                        
                        if ($this->config->get('hips_mode_bar') == 'yes') {
                            
                            $this->model_extension_payment_hips_checkout->addShipping($hips_checkout_order['order_id'], $order);
                        }
                        ...
                        if ($order) {
                            ...
                            $payment_country_info  = $this->model_extension_payment_hips_checkout->getCountryByIsoCode2($order->billing_address->country);
                            $shipping_country_info = $this->model_extension_payment_hips_checkout->getCountryByIsoCode2($order->shipping_address->country);
                            ...
                            if ($payment_country_info && isset($order->billing_address->region)) {
                                $payment_zone_info = $this->model_extension_payment_hips_checkout->getZoneByCode($order->billing_address->region, $payment_country_info['country_id']);
                            }
                            ...
                            $this->model_extension_payment_hips_checkout->updateOcOrder($hips_checkout_order['order_id'], $order_data);
                            ...
                            $jresult = $this->model_extension_payment_hips_checkout->getPaymentInfo($resValue, $this->config->get('hips_key'));
                            $message = '';
                            ...
                            $this->model_extension_payment_hips_checkout->log('Cannot retrieve HC order using order_id: ' . $this->request->get['hips_order_id']);
                        }
                    }
                    
                }
            } else {
                $this->model_extension_payment_hips_checkout->log('Cannot find HC order using order_id: ' . $this->request->get['hips_order_id']);
    ...
    public function left()
    {
        ...
        return $this->load->view('extension/payment/hips_checkout_left', $data);
    ...
    public function leftHTML()
    {
        ...
        $this->response->setOutput($this->load->view('extension/payment/hips_checkout_left', $data));
    ...
    private function hipsOrderData()
    {
        ...
        $merchant_urls = array(
            ...
            'user_return_url_on_fail' => html_entity_decode($this->url->link('extension/payment/hips_checkout/fail', 'hips_order_id={checkout.order.id}', true)),
            'webhook_url' => html_entity_decode($this->url->link('extension/payment/hips_checkout/callback', 'hips_order_id={checkout.order.id}', true))
        );
    ...
    public function send()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            $jresult = $this->model_extension_payment_hips_checkout->paymentInfo($cardDetail, $this->config->get('hips_key'));
            $message = '';
